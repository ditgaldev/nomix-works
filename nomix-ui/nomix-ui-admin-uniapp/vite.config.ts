...
/** Nomix UI 组件解析器：本地 @/components/nomix-ui/* 的组件 */
function NomixUiResolver(): ComponentResolver {
  return {
    type: 'component',
    resolve: (name: string) => {
      if (name.match(/^Yd[A-Z]/)) {
        ...
        return {
          name,
          from: `@/components/nomix-ui/${compName}/${compName}.vue`,
        }
      }
    },
  }
