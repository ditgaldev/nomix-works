  ...
  () => {
    ...
    const setTokenInfo = (val: IAuthLoginRes) => {
      ...
      if (isSingleTokenRes(val)) {
        ...
        // add by Nomix：目前后端没有返回 refreshToken 的过期时间，所以这里暂时不存储 refreshToken 过期时间
      }
    }
    ...
    const isRefreshTokenExpired = computed(() => {
      ...
      // add by Nomix：目前后端没有返回 refreshToken 的过期时间，所以这里暂时不做过期判断，先全部返回 false 非过期
      return false
    })
    ...
    const login = async (loginForm: ILoginForm) => {
      ...
      try {
        ...
        // 注释 by Nomix：避免覆盖 http.ts 中的错误提示
        // uni.showToast({
    ...
    const hasValidLogin = computed(() => {
      ...
      if (isDoubleTokenMode) {
        // add by Nomix：双令牌场景下，以刷新令牌过期为准。而刷新令牌是否过期，通过请求时返回 401 来判断（由于后端 refreshToken 不返回过期时间）
