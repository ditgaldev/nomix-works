...
foreach ($container in @('nomix-htzx-mysql','nomix-htzx-redis')) {
...
$started = @()
try {
    $started += Start-LocalService 'backend' 48080 $java @('-Dfile.encoding=UTF-8','-Xms256m','-Xmx1024m','-jar','nomix-server/target/nomix-server.jar','--server.address=127.0.0.1','--spring.redis.port=6380','--spring.boot.admin.client.enabled=false') $projectRoot
    $env:VITE_OPEN = 'false'
